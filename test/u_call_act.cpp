...
TEST(unit_call_act, exec_callback) {
	...
	auto opt = make_shared < _call_act_t < unit_call_settings > >(&unit_call_act_callback);
	unit_call_settings settings = {};

	opt->exec(_range_t(), settings);
...
TEST(unit_call_act, exec_null_callback) {
	...
	auto opt = make_shared < _call_act_t < unit_call_settings > >(nullptr);
	unit_call_settings settings = {};
